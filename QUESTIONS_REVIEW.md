# Quiz Questions -- Review and Edit

Edit any question, answer, explanation, or difficulty below. Once you're happy, let me know and I'll update the app.

**How to read each question:**

- **Type**: `multiple-choice` or `true-or-false` (more types coming in Phase 2: drag-and-drop, fill-in-the-blank, matching-pairs, create-it, scenario, order-it)
- **Difficulty**: 1 (easy, 10pts), 2 (medium, 20pts), 3 (hard, 30pts)
- **Answer (multiple-choice)**: The letter of the correct option (A, B, C, or D)
- **Answer (true-or-false)**: True or False

Feel free to add, remove, reword, or change difficulty levels. You can also add notes in the margins about questions you'd like for the other activity types (drag-and-drop, create-it, order-it, etc.).

---

## Topic: Energy & Macronutrients (8 questions)

### E1. (Multiple Choice, Difficulty: 1)

**Question:** Which macronutrient is the body's preferred source of energy?

- A) Protein
- B) Carbohydrates ✅
- C) Fat
- D) Vitamins

**Explanation:** Carbohydrates are broken down into glucose which fuels every cell, especially the brain.

---

### BQ2. (Multiple Choice, Difficulty: 1)

**Question:** Why are carbohydrates, protein, and fat called "macronutrients"?

- A) Because they are the most expensive nutrients
- B) Because we consume them in the largest quantities ✅
- C) Because they are only found in large foods
- D) Because they were discovered first

**Explanation:** Macro means large! Macronutrients are nutrients we consume in the large quantities compared to other nutrients.

---

### E4. (Sort, Difficulty: 1)
**Question:** Sort the macronutrients into the correct boxes:

Note to agent: We will use the "Sort the macronurtient" game from the Nutrition App folder.

---
### E4. (True or False, Difficulty: 1)

**Statement:** Fats are a quicker energy source than carbohydrates.

**Answer:** False ✅

**Explanation:** Carbohydrates are used preferentially over protein and fats by the body for energy, especially for the brain.

---

### E5. (Multiple Choice, Difficulty: 2)

**Question:** Which macronutrient is especially important for fuelling the brain?

- A) Fat
- B) Protein
- C) Carbohydrates ✅
- D) Fibre

**Explanation:** Carbohydrates provide the main energy source particularly for the brain.

---

### E6. (Multiple Choice, Difficulty: 1)

**Question:** A balanced meal should include which food groups?

- A) Only carbohydrates
- B) Protein and carbohydrates only
- C) Protein, carbohydrates, and fats ✅
- D) Only protein and fats

**Explanation:** Aiming for 80/20 approach means eating whole foods (veggies, non-processed meats, whole grains) about 80% of the time.

---

## Topic: Blood Sugar (9 questions)

### BS1. (Multiple Choice, Difficulty: 1)

**Question:** When we eat carbohydrates, they are broken down into which sugar?

- A) Fructose
- B) Sucrose
- C) Glucose ✅
- D) Lactose

**Explanation:** Carbohydrates are broken down into glucose, which is used by the cells of the body to make energy.

---

### BS2. (True or False, Difficulty: 2)

**Statement:** Too much glucose in the bloodstream is _______ to the body.

**Select the correct word:** 
- A) Energizing
- B) Toxic ✅
- C) Sleep-promoting

**Explanation:** Yes! That's why blood sugar levels are tightly regulated by the hormone insulin.

---

### BS3. (Multiple Choice, Difficulty: 1)

**Question:** Which hormone regulates blood sugar levels?

- A) Adrenaline
- B) Insulin ✅
- C) Cortisol
- D) Serotonin

**Explanation:** Insulin is released from the pancreas to bring blood sugar levels within normal limits after eating.

---

### BS8. (Multiple Choice, Difficulty: 2)

**Question:** Where is excess glucose stored in the body?

- A) Only in the brain
- B) In liver and muscle cells, and the extra in fat cells ✅
- C) Only in the stomach
- D) It is immediately used up

**Explanation:** Glucose is stored in liver and muscle cells. When there's too much, the excess is stored in adipose (fat) cells.

---

## Topic: The 9 Steps (12 questions)

### S1. (Multiple Choice, Difficulty: 1) — Step 1: Get Choosy with Carbs

**Question:** Select appropriate carb-swaps for the carbohydrate shown:

- Image 1) White bread: Drag and drop brown bread icon
- Image 2) Sugary cereal image (colorful cereals): Drag and drop oatmeal bowl. 
- Image 3) Energy drink image: Drag and drop a fruit smoothie. 
- Image 4) Pancakes with syrup image: Drag and drop image of pancakes with fruits and berries. 


**Explanation:** Swapping refined carbs (like white bread) for whole grain alternatives and including fibre from veg, fruits, and legumes is a key strategy for stabilizing blood sugar.

---

### S2. (True or False, Difficulty: 1) — Step 1: Get Choosy with Carbs

**Statement:** Add up to four ingredients to create a tasty alternative to sugary fizzy drinks.

**Drag and Drop:** Drag the ingredient into the cup to create a new drink. 
- Apple juice 
- Orange juice 
- Water 
- Lemon
- Berries
- Mint leaves
- Watermelon
- Mango juice

**Explanation:** Replacing high sugar drinks can help stabilize insulin levels and provide steadier energy levels.

---

### S3. (Multiple Choice, Difficulty: 2) — Step 2: Eat Protein with Every Meal

**Question:** Why is it a good idea to eat a source of protein with every meal?

- A) Protein tastes better than other foods
- B) Protein takes longer to break down, giving gradual energy release ✅
- C) Protein has more calories
- D) Protein only helps with sleep

**Explanation:** Protein digests more slowly than carbohydrates, providing gradual energy release and satiety (feeling full).

---

### S4. (True or False, Difficulty: 2) — Step 3: Consume Health-Promoting Fats

**Statement:** Fats help your body absorb vitamins _ _ _ and _.

**Select the correct answers:** 
- A ✅
- D ✅
- C
- E ✅
- B12
- K ✅

**Explanation:** Health-promoting fats play a key role in absorbing and storing the fat-soluble vitamins A, D, E and K.

---

### S5. (Multiple Choice, Difficulty: 2) — Step 3: Consume Health-Promoting Fats

**Question:** Which of these foods contain health-promoting fats?

**Answer:** 
- Potato chips 
- Avocado ✅
- Olives ✅
- Donuts
- Almonds ✅
- Olive oil ✅
- Cookie
- French fries
- Salmon ✅

**Explanation:** Essential fats are found in foods like nuts, seeds, salmon, mackerel, avocados, and olive oil. They support brain function and mental health, among many other benefits.

---

### S6. (True or False, Difficulty: 2) — Step 4: Hydrate!

**Statement:** Dehydration can impact blood sugar regulation.

**Answer:** True ✅

**Explanation:** Reduced water in the bloodstream increases the concentration of glucose. Drinking water consistently helps to keep blood sugar in a healthy range.

---

### S7. (Multiple Choice, Difficulty: 2) — Step 4: Hydrate!

**Question:** Being well hydrated improves:

**Select all that apply:** 
- A) Temperature regulation ✅
- B) Concentration ✅
- C) Decision making ✅
- D) Food flavor
- E) Sports performance ✅
- F) Nutrient transport ✅

**Explanation:** Being well hydrated is essential for bodily functions!

---

### S9. (Multiple Choice, Difficulty: 1) — Step 6: Nutritious Snacks Between Meals

**Question:** Which steps can help keep blood sugar levels well-managed throughout the day?

**Select all that apply:** 
- A) Starting the day with breakfast ✅
- B) High sugar drinks between meals
- C) Three main meals in the day (+1-3 snacks if needed) ✅
- D) Daily exercise ✅
- E) Dessert after each meal
- F) Pairing protein with carbs ✅
- G) Eating fibre-rich foods ✅
- H) Snacking on sweets

**Explanation:** Consistent healthy eating habits help to prevent blood sugar spikes and support long-term health.


### S11. (Multiple Choice, Difficulty: 2) — Step 8: Move Your Body!

**Question:** How does exercise help with blood sugar?

- A) It has no effect on blood sugar
- B) Muscles use sugar for energy, helping lower blood sugar levels ✅
- C) It increases blood sugar rapidly
- D) It only helps if you exercise for over 2 hours

**Explanation:** Regular physical activity helps control blood sugar levels — muscles use sugar for energy, and exercise helps your body use insulin more efficiently.

---

### S12. (True or False, Difficulty: 1) — Step 9: Relax and De-Stress

**Question:** Which activities can help to reduce stress?

**Answer:** 
- Running a marathon
- Listening to music ✅
- Sleep ✅
- Taking an exam
- Reading ✅
- Airport 
- Hug ✅ 

**Explanation:** Finding ways to relax and de-stress is important for managing energy.

---

## Simple vs Complex Carbs

### BQ3. (Multiple Choice, Difficulty: 2)

**Question:** Sort the following foods between refined carbohydrates (simple carbs) and complex carbohydrates: 

**Drag and Drop:** 
Simple carbs (use an image for each food)
- White bread ✅ 
- Sugar cereal ✅
- Apple juice ✅
- Donut ✅

Complex carbs:
- Brown bread ✅
- Potato ✅
- Broccoli ✅
- Banana ✅

**Explanation:** Refined carbohydrates are digested and absorbed quickly, causing a sharp rise in blood glucose. Complex carbohydrates take longer to digest, releasing glucose more slowly.

---
### BQ6. (Multiple Choice, Difficulty: 2)

**Question:** Which of the following is NOT another name for sugar on a food label?

- A) Dextrose
- B) Maltodextrin
- C) Molasses
- D) Calcium ✅

**Explanation:** Sugar can hide under many names on food labels including dextrose, glucose, fructose, maltose, sucrose, syrup, maltodextrin, and molasses. Calcium is a mineral, not a sugar.

### BQ7. (Multiple Choice, Difficulty: 3)

**Question:** When reading a food label, how many grams of sugar per 100g is considered "high in sugar"?

- A) 10g per 100g ✅
- B) 5g per 100g
- C) 2g per 100g
- D) 20g per 100g

**Explanation:** As a rough guide, 10g of sugar per 100g is considered high in sugar, while 2g per 100g is considered low. One teaspoon of sugar equals about 4g.

---

### BQ9. (True or False, Difficulty: 2)

**Statement:** No more than 5% of our daily energy should come from sugars, which equals about 30g.

**Answer:** True ✅

**Explanation:** The recommendation is that no more than 5% of daily energy intake should come from added sugars, which works out to roughly 30g per day.

---

## Protein

### BQ10. (Multiple Choice, Difficulty: 1)

**Question:** Protein is vital for which of the following?

**Select all that apply:**
- A) Building muscles ✅
- B) Growth and repair of tissues ✅
- C) Providing quick energy
- D) Making enzymes ✅
- E) Supporting the immune system  ✅
- F) Improving hydration levels

**Explanation:** Protein forms the building blocks for muscles and tissues, enzymes for digestion, and antibodies for our immune system. It's vital for growth and repair, and is also important for healthy hair and skin.

---

### BQ11. (True or False, Difficulty: 1)

**Statement:** Including protein with carbohydrates hinders digestion and raises blood glucose quickly.

**Answer:** True ✅

**Explanation:** Protein is digested more slowly than carbohydrate. When eaten together, the meal is digested more slowly than carbohydrate alone, supporting better blood glucose balance and steadier energy.

---
## Vitamins, Minerals, and the Rainbow

### BQ14. (Multiple Choice, Difficulty: 1)

**Question:** Why is it good to eat fruits and vegetables of many different colours?

- A) It makes your plate look nicer
- B) Different colours provide different vitamins and minerals ✅
- C) Colourful foods always taste better
- D) It doesn't matter what colour they are

**Explanation:** Different types and colours of fruits and vegetables provide different vitamins and minerals. The more different colours you eat, the wider the range of nutrients you get.

---
## Snacking

### BQ17. (True or False, Difficulty: 1)

**Fill in the correct word:** Eating something sweet _____ a balanced meal, when your stomach is ______, slows the absorption of ________ into your blood and helps avoid the blood sugar __________.


**Drag and drop words:** 
- after
- full
- glucose
- roller coaster.

---
### BQ20. (Multiple Choice, Difficulty: 1)

**Question:** About what percentage of our body is made of water?

- A) 20%
- B) 40%
- C) 60% ✅
- D) 80%

**Explanation:** About 60% of our body is made of water. We lose water through sweat, especially during exercise and hot weather. Aim for 6-8 glasses or 1.2 litres of water every day.

---

## Notes for Future Activity Types

_Use this space to jot down ideas for questions that would work well with the other activity types we'll build in Phase 2. For example:_

### Drag-and-Drop Ideas

- Sort foods into macronutrient categories (carbs / protein / fats)
- Sort carbs into "whole grain" vs "refined"

### Create-It Ideas

- Build a balanced breakfast plate (drag carbs, protein, and fats onto a plate)
- Create a lower-sugar drink (drag ingredients into a cup/bottle)
- Create a DIY sports drink

### Order-It Ideas

- Put the blood sugar regulation process in order (eat carbs → broken down to glucose → blood sugar rises → insulin released → glucose stored → blood sugar returns to normal)
- Put the "blood sugar roller coaster" cycle in order

### Fill-in-the-Blank Ideas

- "Carbohydrates are broken down into a sugar called ______." (glucose)
- "Blood sugar levels are regulated by the hormone ______." (insulin)
- "Insulin is released from the ______." (pancreas)

### Matching Pairs Ideas

- Match each of the 9 steps to its key benefit
- Match foods to their macronutrient group
- Match hydration signs to what they mean

### Scenario Ideas

- "Amara has a football match at 2pm. It's now 11am. What should she eat?"
- "Kofi feels tired and can't concentrate in class after lunch. What might help?"
- "Priya is about to go for a run. Should she have a sports drink before or after warming up?"
